...
void VehicleDataProvider::update(const traci::VehicleController* controller)
{
	...
	if (delta > 0.0 * seconds) {
		...
		//start ryu
		// std::string csv_file_path = "data/stationIdList.csv";
		// std::ofstream ofs;
		// ofs.open(csv_file_path,std::ios::app);
		// ofs << "station_id," << mStationId << ",nodeID," << controller->getVehicleId()<< ",typeID," << controller->getTypeId()<< ",class," << controller->getVehicleClass()<<std::endl;
		// ofs.close();
		//end ryu
	} else {
