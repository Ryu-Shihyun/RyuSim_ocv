...
void LocalDynamicMap::updateAwareness(const CaObject& obj)
{
    ...
    // std::cout << "stationID:" << msg->header.stationID << ", tai:" << mTimer.getTimeFor(tai) << "\n";
...
//ryu added
omnetpp::SimTime LocalDynamicMap::getTime(const CaObject& obj)
{
    const vanetza::asn1::Cam& msg = obj.asn1();

    auto tai = mTimer.reconstructMilliseconds(msg->cam.generationDeltaTime);
    
    return mTimer.getTimeFor(tai); 
}
