...
void LtePhyVUeMode4::initialize(int stage)
{
    ...
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        //start ryu
        tbDecodedPositionX                  = registerSignal("tbDecodedPositionX");
        tbDecodedPositionY                  = registerSignal("tbDecodedPositionY");
        setRRI                              = registerSignal("setRRI");
        //end ryu
    ...
    if (msg->isName("d2dDecodingTimer"))
    {
        ...
        while (!tbInfo_.empty()){
            ...
            if(missingPosition != missingTbs.end()) {
                ...
                //start ryu
                emit(tbDecodedPositionX,-1);
                emit(tbDecodedPositionY,-1);
                //end ryu
            } else {
                ...
                //start ryu
                // std::string csv_file_path = "data/tbDecoded.csv";
                // std::ofstream ofs;
                // ofs.open(csv_file_path,std::ios::app);
                // ofs << NOW.format(-9)<< "," << nodeId_ << "," << tbDecoded_ << std::endl;
                // ofs.close();
                //end ryu
...
SidelinkControlInformation* LtePhyVUeMode4::createSCIMessage()
{
    ...
    if (sciGrant_->getExpiration() != 0)
    {
        if (sciGrant_->getPeriod() == 50){
            ...
            emit(setRRI,sciGrant_->getPeriod() / 100);
        }
    }
    else
    {
...
void LtePhyVUeMode4::decodeAirFrame(LteAirFrame* frame, UserControlInfo* lteInfo, std::vector<double> &rsrpVector, std::vector<double> &rssiVector, std::vector<double> &sinrVector, double &attenuation)
{
    ...
    else
    {
        ...
        //start ryu
        // std::string csv_file_path = "data/txRxDistanceTB.csv";
        // std::ofstream ofs;
        // ofs.open(csv_file_path,std::ios::app);
        // ofs << NOW.format(-9)<< "," << nodeId_ << "," << pkt_dist << std::endl;
        // ofs.close();
        //end ryu
        //start ryu
        emit(tbDecodedPositionX,getCoord().x);
        emit(tbDecodedPositionY,getCoord().y);
        // std::cout << getCoord().x << "," << getCoord().y << "\n";
        //end ryu
