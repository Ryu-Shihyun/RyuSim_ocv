...
void LteMacVUeMode4::handleMessage(cMessage *msg)
{
    ...
    else if (incoming == up_[IN])
    {
        if (strcmp(pkt->getName(), "newDataPkt")== 0)
        {
            ...
            // if (nodeId_ == 1026) 
                // std::cout << NOW << " strcmp(pkt->getName(), newDataPkt)== 0, getDuration():" << lteInfo->getDuration() << ", remainingTime_:"<< remainingTime_  << ", periodCounter_:" << periodCounter_ << endl; //test by ryu
            if (schedulingGrant_ == NULL)
            {
                // if (nodeId_ == 1026) 
            ...
            {
                // if (nodeId_ == 1026) 
                    // std::cout << NOW << " else "  << endl; //test by ryu
                LteMode4SchedulingGrant* mode4Grant = check_and_cast<LteMode4SchedulingGrant*>(schedulingGrant_);
...
void LteMacVUeMode4::handleSelfMessage()
{
    ...
    // if (nodeId_ == 1026) 
        // std::cout << NOW << " LteMacVUeMode4::handleSelfMessage Purged " << purged << " PDUS" << endl; //test by ryu

    // if (nodeId_ == 1026) 
        // std::cout << NOW << "LteMacVUeMode4::handleSelfMessage " << nodeId_ << " - HARQ process " << (unsigned int)currentHarq_ << endl; //test by ryu
    ...
    if (mode4Grant == NULL)
    {
        ...
        // if (nodeId_ == 1026) 
            // cout << NOW << " LteMacVUeMode4::handleSelfMessage " << nodeId_ << " NO configured grant" << endl;//test by ryu
    ...
    else if (mode4Grant->getPeriodic() && mode4Grant->getStartTime() <= NOW)
    {
        ...
        if(--expirationCounter_ == mode4Grant->getPeriod())
        {
            ...
            if (randomReReserve < probResourceKeep_)
            {
                ...
                emit(retainGrant, 1);
                emit(rriValue,resourceReservationInterval_);
                // if (nodeId_ == 1026) 
                    // cout << NOW << " randomReReserve < probResourceKeep_" << endl;//test by ryu
        ...
        else if (expirationCounter_ <= 0)
        {
            ...
            expiredGrant_ = true;
            // if (nodeId_ == 1026) 
                // cout << NOW << " expirationCounter_ <= 0, expirationCounter_:" << expirationCounter_ << endl;//test by ryu
        }
    }
    ...
    {
        // if (nodeId_ == 1026) 
            // cout << NOW << " mode4Grant!=NULL && mode4Grant->getStartTime() <= NOW" << endl;//test by ryu
        if (mode4Grant->getFirstTransmission())
        {
            mode4Grant->setFirstTransmission(false);
            // if (nodeId_ == 1026) 
                // cout << NOW << " mode4Grant->getFirstTransmission()" << endl;//test by ryu
        }
        if(!firstTx)
        {
            EV << "\t currentHarq_ counter initialized " << endl;
            // if (nodeId_ == 1026) 
                // cout << "\t currentHarq_ counter initialized " << endl;//test by ryu
            firstTx=true;
        ...
        // if (nodeId_ == 1026) 
            // cout << "\t " << schedulingGrant_ << endl;//test by ryu

        // if (nodeId_ == 1026) 
            // cout << NOW << " LteMacVUeMode4::handleSelfMessage " << nodeId_ << " entered scheduling" << endl;//test by ryu
        ...
        for(it2 = harqTxBuffers_.begin(); it2 != harqTxBuffers_.end(); it2++)
        {
            ...
            // if (nodeId_ == 1026) 
                // cout << "\t Looking for retx in acid " << (unsigned int)currentHarq_ << endl;//test by ryu
            currHarq = it2->second;
            ...
            {
                // if (nodeId_ == 1026) 
                    // cout << "it2->second->isSelected()" << endl;//test by ryu
            ...
            // if (nodeId_ == 1026) 
                // cout << "\t [process=" << (unsigned int)currentHarq_ << "] , [retx=" << ((retx)?"true":"false") << "] , [n=" << cwListRetx.size() << "]" << endl;//test by ryu

            // if a retransmission is needed
            if(retx)
            {
                // if (nodeId_ == 1026) 
                    // cout << "retx" << endl;//test by ryu
                UnitList signal;
        ...
        {
            // if (nodeId_ == 1026) 
                // cout << "!retx && !availablePdu" << endl;//test by ryu
        ...
        // if (nodeId_ == 1026) 
            // cout << "scheduleAt(NOW, flushHarqMsg);" << endl;//test by ryu
    }
    //============================ DEBUG ==========================
    ...
    // if (nodeId_ == 1026) 
        // cout << "\n htxbuf.size " << harqTxBuffers_.size() << endl;//test by ryu
    ...
    for(it = harqTxBuffers_.begin(); it != harqTxBuffers_.end(); it++)
    {
        ...
        // if (nodeId_ == 1026) 
            // cout << "\t cicloOuter " << cntOuter << " - bufferStatus.size=" << harqStatus.size() << endl;//test by ryu
        for(; jt != jet; ++jt)
        {
            EV_DEBUG << "\t\t cicloInner " << cntInner << " - jt->size=" << jt->size()
               << " - statusCw(0/1)=" << jt->at(0).second << "/" << jt->at(1).second << endl;
            // if (nodeId_ == 1026) 
                // cout << "\t\t cicloInner " << cntInner << " - jt->size=" << jt->size() << " - statusCw(0/1)=" << jt->at(0).second << "/" << jt->at(1).second << endl;//test by ryu
        }
    }
    //======================== END DEBUG ==========================
    ...
    {
        // if (nodeId_ == 1026) 
            // cout << "!requestSdu" << endl;//test by ryu
...
void LteMacVUeMode4::changeRRIbyTChange(float time){
    ...
    resourceReservationInterval_ = newRRI;
}